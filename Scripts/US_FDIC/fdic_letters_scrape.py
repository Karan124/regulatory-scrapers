"""
FDIC Financial Institution Letters Scraper
Production-ready script for scraping FDIC FILs with LLM-friendly output.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('fdic_letters_scraper.log'),
...
class FDICLettersScraper:
    """Scraper for FDIC Financial Institution Letters with anti-bot measures and incremental updates."""
    
    BASE_URL = "https://www.fdic.gov"
    LETTERS_URL = f"{BASE_URL}/news/financial-institution-letters"
    OUTPUT_FILE = Path("data/fdic_letters.json")
    ...
    def load_existing_data(self) -> List[Dict]:
        """Load existing letters to avoid re-scraping."""
        ...
        try:
            with open(self.OUTPUT_FILE, 'r', encoding='utf-8') as f:
                ...
                logger.info(f"Loaded {len(data)} existing letters.")
                return data
    ...
    def save_data(self, data: List[Dict]):
        ...
        logger.info(f"Saved {len(data)} letters to {self.OUTPUT_FILE}")
    ...
    async def get_total_pages(self) -> int:
        ...
        html = await self.fetch_with_retry(self.LETTERS_URL)
        if not html:
            return 1
    ...
    async def scrape_index_page(self, page_num: int) -> List[Dict]:
        """Scrape a single index page for letter links."""
        url = f"{self.LETTERS_URL}?pg={page_num}" if page_num > 1 else self.LETTERS_URL
        ...
        logger.info(f"Page {page_num}: Found {len(letters)} new letters")
        return letters
    ...
    async def scrape_article(self, basic_info: Dict) -> Optional[Dict]:
        ...
        logger.info(f"Scraping letter: {basic_info['headline']}")
        ...
        release_type = "Joint Release" if joint_release else "Financial Institution Letter"
    ...
    async def run(self):
        ...
        logger.info("=" * 60)
        logger.info("Starting FDIC Financial Institution Letters Scraper")
        logger.info("=" * 60)
        ...
        for page_num in range(1, total_pages + 1):
            letters = await self.scrape_index_page(page_num)
            all_basic_info.extend(letters)
            await asyncio.sleep(self.rate_limit_delay)
        
        logger.info(f"Found {len(all_basic_info)} new letters to scrape")
        
        if not all_basic_info:
            logger.info("No new letters to scrape. Exiting.")
            return
        
        # Scrape full articles
        new_articles = []
        for i, basic_info in enumerate(all_basic_info, 1):
            logger.info(f"Processing letter {i}/{len(all_basic_info)}")
        ...
        logger.info(f"Total letters in dataset: {len(all_data)}")
        logger.info(f"New letters added: {len(new_articles)}")
...
async def main():
    ...
    async with FDICLettersScraper() as scraper:
        await scraper.run()
