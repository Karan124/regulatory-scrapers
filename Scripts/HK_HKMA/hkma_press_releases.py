#!/usr/bin/env python3
"""
HKMA Press Release Scraper
Production-grade scraper for Hong Kong Monetary Authority press releases
...
CONFIG = {
    ...
    "press_releases_url": "https://www.hkma.gov.hk/eng/news-and-media/press-releases/",
    "start_date": "2025-09-01",  # Configurable: only scrape articles on or after this date
    "output_file": "data/hkma_press_releases.json",
    "delay_range": (2, 5),  # Random delay between requests (seconds)
    "max_retries": 3,
    "timeout": 30000,  # Playwright timeout in ms
    "incremental_save_interval": 10,  # Save every N articles
}
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('hkma_scraper.log'),
...
class HKMAScraper:
    """Main scraper class for HKMA press releases."""
    
    def __init__(self):
        self.base_url = CONFIG["base_url"]
        self.press_releases_url = CONFIG["press_releases_url"]
    ...
    def scrape_index_page(self) -> List[Dict]:
        """Scrape the press releases index page with dynamic loading."""
        logger.info("Starting to scrape press releases index page")
        articles = []
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                logger.info(f"Navigating to press releases page: {self.press_releases_url}")
                page.goto(self.press_releases_url, wait_until='domcontentloaded', timeout=CONFIG["timeout"])
                page.wait_for_timeout(3000)
                
                load_more_clicks = 0
                stop_loading = False
                
                # Load more articles until we reach the start date
                while not stop_loading:
                    # Parse current page content
                    html = page.content()
                    soup = BeautifulSoup(html, 'html.parser')
                    press_release_div = soup.find('div', {'id': 'press-release-result'})
                    
                    if not press_release_div:
                        logger.warning("Press release container not found")
                        break
                    
                    # Find all <ul> elements containing press releases
                    ul_elements = press_release_div.find_all('ul')
                    logger.info(f"Found {len(ul_elements)} press release entries on current page")
                    
                    for ul in ul_elements:
                        li_elements = ul.find_all('li')
                        
                        # Each press release has 2 <li> elements: date and link
                        if len(li_elements) >= 2:
                            ...
                            if link_elem:
                                ...
                                # Check if article is before our start date
                                if article_date < self.start_date:
                                    logger.info(f"Reached articles before start date: {date_text} < {self.start_date.strftime('%Y-%m-%d')}")
                                    stop_loading = True
                                    break
                                ...
                                logger.info(f"Found new article [{len(articles)}]: {title} ({article_date.strftime('%Y-%m-%d')})")
                    ...
                    try:
                        ...
                        if load_more_btn.is_visible(timeout=3000):
                            ...
                            logger.info("'Load More' button not visible - all articles loaded")
                            break
                            
                    except PlaywrightTimeout:
                        logger.info("'Load More' button timeout - assuming all articles loaded")
                        break
                    except Exception as e:
        ...
        logger.info(f"Index page scraping complete: Found {len(articles)} new articles")
        return articles
    
    def scrape_article(self, article_info: Dict) -> Optional[Dict]:
        """Scrape a single article page with retry logic."""
        ...
        for attempt in range(1, CONFIG["max_retries"] + 1):
            try:
                ...
                # Infer theme/type from title and content
                theme = self._infer_theme(article_info['title'], body_text)
                ...
                logger.info(f"Successfully scraped article with {len(tables)} tables and {len(attachments)} attachments")
                self._random_delay()
                return article_data
                ...
                logger.error(f"Error scraping article {url} (attempt {attempt}): {e}", exc_info=True)
                if attempt < CONFIG["max_retries"]:
    ...
    def _infer_theme(self, title: str, body: str) -> str:
        """Infer theme/type from title and content using keyword matching."""
        text = (title + " " + body[:500]).lower()  # Use title + first 500 chars
        
        # Theme keywords (order matters - more specific first)
        themes = {
            'Scam Alert': ['scam', 'fraud', 'phishing', 'suspicious', 'fraudulent'],
            'Bond Issuance': ['bond', 'tender', 'issuance', 'sovereign bond', 'government bond'],
            'Exchange Fund': ['exchange fund', 'foreign reserve', 'analytical accounts', 'foreign currency reserve'],
            'Monetary Policy': ['monetary policy', 'interest rate', 'base rate', 'money supply', 'discount window'],
            'Banking Stability': ['banking stability', 'financial stability', 'systemic risk', 'prudential'],
            'Regulatory Developments': ['regulatory', 'regulation', 'supervision', 'guideline', 'circular', 'compliance'],
            'FinTech & Innovation': ['fintech', 'innovation', 'sandbox', 'technology', 'digital', 'a.i.', 'artificial intelligence'],
            'Climate Finance': ['climate', 'sustainable', 'green finance', 'esg', 'environmental'],
            'International Cooperation': ['cooperation', 'conference', 'collaboration', 'partnership', 'mou', 'memorandum'],
        }
    ...
    def _save_incremental(self, new_articles: List[Dict]):
        """Save accumulated articles incrementally (append to existing)."""
        try:
            # Combine existing + new articles
            all_data = self.existing_data + new_articles
            
            # Write to file
            with open(self.output_file, 'w', encoding='utf-8') as f:
                json.dump(all_data, f, ensure_ascii=False, indent=2)
            
            # Update existing_data reference to include newly saved articles
            ...
            logger.info(f"Incremental save: {len(new_articles)} new articles saved (total: {len(all_data)})")
    ...
    def run(self):
        ...
        logger.info("=" * 80)
        logger.info("HKMA Press Release Scraper - Starting")
        ...
        # Step 1: Scrape index page to get list of articles
        articles_to_scrape = self.scrape_index_page()
        
        if not articles_to_scrape:
            logger.info("No new articles to scrape - exiting")
            return
        
        logger.info(f"Will scrape {len(articles_to_scrape)} new articles")
        ...
        for i, article_info in enumerate(articles_to_scrape, 1):
            logger.info(f"\n--- Processing article {i}/{len(articles_to_scrape)} ---")
            article_data = self.scrape_article(article_info)
            
            if article_data:
                successfully_scraped.append(article_data)
                ...
                failed_urls.append(article_info['url'])
                logger.warning(f"Failed to scrape: {article_info['url']}")
        ...
        logger.info(f"Total articles in database: {len(self.existing_data)}")
        logger.info(f"Successfully scraped: {len(articles_to_scrape) - len(failed_urls)}")
